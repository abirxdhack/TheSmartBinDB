"""Sphinx configuration for SmartBinDB documentation."""

import os
import re
import sys

sys.path.insert(0, os.path.abspath("../.."))

with open(os.path.abspath("../../smartbindb/__init__.py"), encoding="utf-8") as _f:
    __version__ = re.search(
        r'^__version__\s*=\s*["\']([^"\']+)["\']',
        _f.read(),
        re.M,
    ).group(1)

project = "SmartBinDB"
author = "Abir Arafat Chawdhury"
copyright = "2024-2026, Abir Arafat Chawdhury"
release = __version__
version = __version__

extensions = [
    "sphinx.ext.autodoc",
    "sphinx.ext.napoleon",
    "sphinx.ext.autosummary",
    "sphinx.ext.intersphinx",
    "sphinx_copybutton",
]

intersphinx_mapping = {"python": ("https://docs.python.org/3", None)}

master_doc = "index"
source_suffix = ".rst"
autodoc_member_order = "bysource"
autosummary_generate = True
autodoc_typehints = "none"

autodoc_default_options = {
    "undoc-members": False,
    "show-inheritance": True,
    "member-order": "bysource",
}

autodoc_inherit_docstrings = True
autodoc_class_signature = "separated"

templates_path = ["../resources/templates"]
html_copy_source = False
napoleon_use_rtype = False
napoleon_use_param = False
pygments_style = "sphinx"
pygments_dark_style = "monokai"
highlight_language = "python3"
copybutton_prompt_text = "$ "

suppress_warnings = [
    "image.not_readable",
    "toctree.not_readable",
]

nitpicky = False

html_title = f"SmartBinDB {version}"
html_theme = "furo"
html_static_path = [os.path.abspath("../../docs_static")]
html_css_files = ["smartbindb.css"]
html_js_files = ["smartbindb_docs.js"]
html_show_sourcelink = False
html_show_copyright = False
html_show_sphinx = False
html_logo = os.path.abspath("../../docs_static/smartbindb.png")
html_favicon = os.path.abspath("../../docs_static/favicon.ico")

html_theme_options = {
    "navigation_with_keys": True,
    "sidebar_hide_name": True,
    "footer_icons": [
        {
            "name": "GitHub",
            "url": "https://github.com/abirxdhack/TheSmartBinDB",
            "html": (
                '<svg stroke="currentColor" fill="currentColor" stroke-width="0" '
                'viewBox="0 0 16 16" height="1em" width="1em" '
                'xmlns="http://www.w3.org/2000/svg">'
                '<path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59'
                ".4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49"
                "-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53"
                ".63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87"
                ".51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15"
                "-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27"
                " 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1"
                ".16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65"
                " 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15"
                '.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z"/>'
                "</svg>"
            ),
            "class": "",
        },
    ],
}

html_sidebars = {
    "**": [
        "sidebar/brand.html",
        "sidebar/search.html",
        "sidebar/scroll-start.html",
        "sidebar/navigation.html",
        "sidebar/version.html",
        "sidebar/scroll-end.html",
    ]
}

html_baseurl = os.environ.get(
    "READTHEDOCS_CANONICAL_URL", "https://abirxdhack.github.io/TheSmartBinDB/"
)

_SKIP_DUNDERS = {
    "__class__", "__delattr__", "__dict__", "__dir__", "__doc__",
    "__eq__", "__format__", "__ge__", "__getattribute__", "__getstate__",
    "__gt__", "__hash__", "__init_subclass__", "__le__", "__lt__",
    "__module__", "__ne__", "__new__", "__reduce__", "__reduce_ex__",
    "__repr__", "__setattr__", "__setstate__", "__sizeof__", "__str__",
    "__subclasshook__", "__weakref__", "__annotations__", "__abstractmethods__",
    "__slots__",
}


def skip_member(app, what, name, obj, skip, options):
    if name in _SKIP_DUNDERS:
        return True
    if name.startswith("_"):
        return True
    if skip:
        return True
    return False


def setup(app):
    app.connect("autodoc-skip-member", skip_member)
