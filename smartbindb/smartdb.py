"""SmartBinDB core implementation with true sync and async support.

This module provides the SmartBinDB class for high-performance offline BIN
lookups. Supports both synchronous and asynchronous method calls with fully
independent implementations for maximum performance and compatibility.
...
class SmartBinDB:
    """High-performance offline BIN lookup with true sync/async support.

    Loads bundled BIN database into memory with O(1) indices. Provides
    independent synchronous and asynchronous implementations for flexible
    integration with any application architecture.
    ...
    async def aget_bin_info(self, bin: str) -> Dict[str, Any]:
        ...
        True async implementation using asyncio for non-blocking operation.
        Defers CPU-bound work to thread pool to avoid blocking event loop.
        ...
        return await loop.run_in_executor(None, self.get_bin_info, bin)
    ...
    async def aget_bins_by_bank(
        ...
        Example:
            .. code-block:: python
                ...
                db = SmartBinDB()
                result = await db.aget_bins_by_bank("Chase", limit=5)
                for row in result["data"]:
                    print(row["bin"], row["issuer"])
        """
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, lambda: self.get_bins_by_bank(bank, limit)
        )
    ...
    async def aget_bins_by_country(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, lambda: self.get_bins_by_country(country, limit)
        )
